...
%
% Attribution:
% This function was copied from GitHub on 23 June 2016.  Thanks to user
% cbraley for making it public.  There was no associated licence file, so
% this attribution will have to do.  See original:
...
%
    ...
    %   BSH: height, not width should be first dimension
    ...
    for i = 1:imHeight
        for j = 1:imWidth
            for k = 1:numChannels
                %   BSH: scan from top down, not bottom up
                img(imHeight - i + 1,j,k) = rawData(idx);
                idx = idx + 1;
            end
        end
    end
